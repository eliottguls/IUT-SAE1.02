...
void quickSort(int tab[], int deb, int fin) {
  if (deb < fin){
      nb_cmp++;
    // trouver le pivot tel que les élements plus petit que le pivot sont à sa gauche, et les éléments à sa droite soit plus grand 
...
int main() {
  ...
  while (strcmp(choix,"croissant") != 0 && (strcmp(choix,"décroissant")!=0)){
    ...
    printf("Temps CPU du tri : %.3f sec\n\n", tmpsCPU);
